...
class Fillpolate {
	...
	expand(list) {
		...
		for (let i = 0; i < sortedIndexes.length - 1; i++) {
			...
			while (offset < width) {
				...
				results.push(Object.assign({}, interpol, {fillpolated: true, [this.indexKey]: offset + index}));
				offset++;
			}
		}
	...
	recursiveInterpolate({previous, next, weightPrevious, schema}) {
		if (!schema.properties) {
			throw (new Error(`properties is mandatory: invalid schema ${JSON.stringify(schema)}`));
		}
		...
		for (const key of Object.keys(schema.properties)) {
			...
			try {
				...
				if (type === 'object' && !hasExplicitStrategy) {
					if(typeof (previous[key]) !== 'object' || typeof (next[key]) !== 'object'){
						if(required){
							throw (new TypeError(`Cannot Interpolate on previous[${key}] / next[${key}] not an object (${previous[key]} / ${next[key]})`));
						}
					} else {
					...
					const interpolatedValue = this.interpolateValue({previous: previous[key], next: next[key], weightPrevious, schema: sch});
					if(typeof(interpolatedValue) !== 'undefined'){
